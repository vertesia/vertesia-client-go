...
func TestCompletionResultGenerationPatch(t *testing.T) {
	dir := t.TempDir()
	...
	tests, err := os.ReadFile("testdata/completion_result_test.go")
	if err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(generated, "completion_result_test.go"), tests, 0600); err != nil {
		t.Fatal(err)
	}
